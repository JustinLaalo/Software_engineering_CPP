...
int testResOverlap(){
    ...
    //tests out different years and checks if they are overlapping. Should not overlap
    Date newDate1(2069,9,30);
 	Date newDate2(2068,9,28);
    Reservation newRes1("Joe", newDate1, 3);
 	Reservation newRes2("Bob", newDate2, 2);

 	std::cout<<endl;
    if (!resOverlap(newRes1, newRes2, false)) score = 0;


    //tests if they start and end on the same day and checks if they are overlapping. Should not overlap
    Date newDate3(2012,9,30);
 	Date newDate4(2012,9,28);
    Reservation newRes3("Joe", newDate3, 1);
 	Reservation newRes4("Bob", newDate4, 1);

 	std::cout<<endl;
    if (!resOverlap(newRes3, newRes4, false)) score = 0;

    //tests long durations and checks if they are overlapping. Should not overlap
    Date newDate5(2012,2,30);
 	Date newDate6(2013,9,28);
    Reservation newRes5("Joe", newDate5, 60);
 	Reservation newRes6("Bob", newDate6, 60);

 	std::cout<<endl;
    if (!resOverlap(newRes5, newRes6, false)) score = 0;

    //tests a regular test case and checks if they are overlapping. Should not overlap
    Date newDate7(2012,8,30);
 	Date newDate8(2012,9,28);
    Reservation newRes7("Joe", newDate7, 2);
 	Reservation newRes8("Bob", newDate7, 2);

 	std::cout<<endl;
    if (!resOverlap(newRes7, newRes8, true)) score = 0;
